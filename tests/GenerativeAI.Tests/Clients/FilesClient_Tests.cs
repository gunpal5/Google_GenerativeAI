...
public class FileClient_Tests : TestBase
{
    ...
    public async Task ShouldGetFileMetadata()
    {
        ...
        var files = await client.ListFilesAsync(cancellationToken: TestContext.Current.CancellationToken);
        var fileX = files.Files.FirstOrDefault();
        var fileName = fileX.Name; // Example file name, replace with test data.
        var file = await client.GetFileAsync(fileName, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ShouldListFiles()
    {
        ...
        var result = await client.ListFilesAsync(pageSize: 5, cancellationToken: TestContext.Current.CancellationToken); // Example: Fetch a maximum of 5 files.
    ...
    public async Task ShouldUploadFileAsync()
    {
        ...
        try
        {
            ...
            // Act
            var result = await client.UploadFileAsync(tempFilePath, progressCallback, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task ShouldDeleteFile()
    {
        ...
        await Should.NotThrowAsync(async () => await client.DeleteFileAsync(fileName, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task ShouldHandleInvalidFileForRetrieve()
    {
        ...
        var exception = await Should.ThrowAsync<Exception>(async () => await client.GetFileAsync(invalidFileName, cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task ShouldHandleInvalidFileForDelete()
    {
        ...
        var exception = await Should.ThrowAsync<Exception>(async () => await client.DeleteFileAsync(invalidFileName, cancellationToken: TestContext.Current.CancellationToken));
        exception.Message.ShouldNotBeNullOrEmpty();
    ...
    public async Task ShouldUploadStream()
    {
        ...
        // Act
        var result = await client.UploadStreamAsync(stream, displayName, mimeType, progressCallback, cancellationToken: TestContext.Current.CancellationToken);
